...
//  if (type >= O_ROOT_ROADN && type <= O_ROOT_ROADS) return 0;
// if (type >= O_GRAVE && type <=O_OBELISK && !(AString(*name) == "Building") && !(AString(*name) == "Dummy") && describe != 0) return 0;
  if (type >= O_GRAVE && type <=O_OBELISK && !(AString(*name).CheckPrefix("Building")) && !(AString(*name) == "Dummy") && describe != 0) return 0; 
...
int Object::IsRoad()
{
    ...
    if (type >= O_ROOT_ROADN && type <= O_ROOT_ROADS) return 1;
    return 0;
}
...
int Object::IsRootRoad()
{
  if (type >= O_ROOT_ROADN && type <= O_ROOT_ROADS) return 1;
  return 0;
}

int Object::CanDestroy() {
  if (num == 0) return 0;
  if (type == O_DUMMY) return 0;
  if (type == O_SHAFT) return 0;
  if (type >= O_ROOT_ROADN && type <= O_ROOT_ROADS) return 0;
