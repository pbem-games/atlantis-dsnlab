...
void Game::WriteReport()
{
    ...
    {//Makes World trifles list
       Trifle_rep=new AList;
       forlist (&regions)
       {
         ARegion * r = (ARegion *) elem;
         forlist(&r->objects) 
         {
           ...
           forlist (&obj->units)
           {
             ...
             forlist (&unit->items)
             {
               Item * it = (Item *) elem;
               if (it->type == I_TRIFLE) 
                 Trifle_rep->Add(new AString(AString(" ")+*it->name));
             }
           }
         }
       }
...
void Game::ClearKilledFars(Unit *u)
{
  forlist(&regions) {
    ARegion * reg = (ARegion *) elem;
    forlist(&reg->farsees) {
      Farsight * fars = (Farsight *) elem;
      if (fars->unit != NULL)
        if (u->num == fars->unit->num) {
          reg->farsees.Remove(fars);
          delete fars;
      }
    }
  }
