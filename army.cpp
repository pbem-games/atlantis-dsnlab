         ...
         if(i->type==I_TRIFLE){
            ...
            for(int x = 0; x < na; x++) {
               ...
               if(u->CanGetSpoil(i)) {
                  ...
                  u->items.SetNum(I_TRIFLE,1);
                  u->items.SetName(I_TRIFLE,i->name);
                  break;
               }
            }
         }
...
// Dweller - added one more parameter (pointer to Battle) for report writong
//int Army::DoAnAttack( int special, int numAttacks, int attackType,
//                      int attackLevel, int flags, int effect )
//  Tails: Catapult
//int Army::DoAnAttack( int special, int numAttacks, int attackType,
//                      int attackLevel, int flags, int effect, Battle* b )
int Army::DoAnAttack( int special, int numAttacks, int attackType,
                      int attackLevel, int flags, int effect, Battle* b, int weap )
// end of Dweller
{
    ...
    // Dweller
    // Ranged weapon has attackType == ATTACK_COMBAT, but
    // it can be deflected by FSHI, so lets try to find that FSHI
    // if attcked by ranged weapon (LBOW,XBOW,DBOW)
    // Original: if( attackType == ATTACK_COMBAT)
    if ( attackType == ATTACK_COMBAT && !( flags & WeaponType::RANGED ) )
    // end of Dweller
    {
        hi = 0;
    } 
    ...
    if (hi)
    {
        ...
        // Dweller
        // Ranged attacks have attack type ATTACK_COMBAT, so we can write 
        // code for destroy FSHI down here
        else if ( hi->shieldtype == ATTACK_COMBAT )
        {
            hi->attackmissed++;

            // this is the chance for FSHI to be destroyed by 1 bow
            if ( getrandom( 1000 ) < 1 ) // <-- it is 0.1% to destroy FSHI by 1 arrow
            {
                if ( b && hi->caster )
                {
                    AString msg;

                    msg  = "Force shield, casted by ";
                    msg += *hi->caster;
                    msg += " has been dissipated after ";
                    msg += hi->attackmissed;
                    msg += " missed and ";
                    msg += hi->attackdeflected;
                    msg += " deflected attacks.";
...
// Dweller - write statistic about participated in battle 
// but not destroyed FSHI at the end of the round
void Army::WriteRemainedFSHIStatistic( Battle* b )
{
    // check only highest shield of ATTACK_COMBAT type
    // (only highest could participate in battle)
    Shield* hi = shields.GetHighShield( ATTACK_COMBAT );
    ...
    if ( hi->attackdeflected    > 0 || 
         hi->attackmissed       > 0 )
    {
        // shield took his part in battle, lets report it
        if ( b && hi->caster )
        {
            AString msg;
            ...
            msg  = "Force shield, casted by ";
            msg += *hi->caster;
            msg += " meet the end of round with ";
            msg += hi->attackmissed;
            msg += " missed and ";
            msg += hi->attackdeflected;
            msg += " deflected attacks.";
